# daily_qqq_fetch_all_timeframe_data.py
# Fetches historical QQQ data for multiple timeframes (daily, 4h, weekly) using yfinance,
# updates existing CSV files, and handles deduplication and time zone consistency.
...
# ----------------------------------
# Define ticker and output directory
ticker = "QQQ"
output_folder = "./qqq_data"

# Define Eastern Time (US market time zone)
ET = pytz.timezone('US/Eastern')

# Configuration for each timeframe: interval, max days, filename, overlap days
configs = {
    "daily_10y": {
        "interval": "1d",
        "max_days": 3650,  # ~10 years
        "filename": f"{output_folder}/qqq_daily_10y.csv",
        "overlap_days": 2
    },
    "4h_729d": {
        "interval": "4h",
        "max_days": 729,  # Yahoo Finance limit for 4h
        "filename": f"{output_folder}/qqq_4h_729d.csv",
        "overlap_days": 1
    },
    "weekly_10y": {
        "interval": "1wk",
        "max_days": 3650,  # ~10 years
        "filename": f"{output_folder}/qqq_weekly_10y.csv",
        "overlap_days": 7
    }
}
...
# ----------------------------------
print("\n📊 Starting QQQ historical data fetch...\n")
...
for label, cfg in configs.items():
    ...
    if last_date is None and not os.path.exists(filename):
        ...
        print(f"📁 No existing data for {label}. Fetching full {max_days}-day range from {format_date(start_date, interval)}.")
        ...
        print(f"🔍 Existing data for {label}. Last saved: {last_date}. Refetching from {format_date(start_date, interval)}.")
    ...
    print(f"⏳ Downloading {label} ({interval}) from {format_date(start_date, interval)} to {format_date(end_date, interval)}...")

    # Fetch new data
    new_data = fetch_new_data(ticker, start_date, end_date, interval)
    if new_data.empty:
        print(f"⚠️ No new data fetched for {label}. Skipping save.\n")
        continue
    ...
    if last_date is not None and not new_data.empty:
        ...
        if new_data.empty:
            print(f"⚠️ No new dates beyond {last_date} for {label}. Skipping save.\n")
            continue
...
print("🎉 All timeframes updated successfully!\n")
